...
function PaymentForm({ onSubmit, onCancel, isOwedMoney = false }) {
        ...
        <div className="form-group">
          ...
          {imagePreview ? (
            ...
            <div className="payment-upload-options">
              <label htmlFor={`paymentImg-${isOwedMoney}`} className="payment-upload-label">
                {uploading ? 'Uploading...' : '📁 Upload'}
                <input id={`paymentImg-${isOwedMoney}`} type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} disabled={uploading} />
              </label>
                            ...
                            {entry.payments.map((payment, index) => (
                              <div key={index} className={`payment-item ${payment.type} clickable-payment`} onClick={() => setSelectedTransaction(payment)}>
                                ...
                                <span className="payment-type">
                                  {payment.type === 'payment' ? '(Paid)' : '(Additional)'}
                                </span>
                                {payment.description && (
